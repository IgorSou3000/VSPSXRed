	...
	if (hit_type == 0)
	{
		for (int i = 0; i < 3; i++)
		{
			...
			Obj_Splash *splash = Obj_Splash_New(
				note_x [type] + FIXED_DEC(bfnotex,1) ,
...
static void Stage_NoteCheck(PlayerState *this, u8 type)
{
	...
	{
		if (!(note->type & (NOTE_FLAG_FIRE | NOTE_FLAG_THUNDER)))
		{
	...
	//Handle player note presses
		...
		if (playing)
		{
			...
			for (Note *note = stage.cur_note;; note++)
			{
				...
				if (note_fp + stage.late_safe < stage.note_scroll)
					continue;
				if ((note->type & NOTE_FLAG_THUNDER) || (note->type & NOTE_FLAG_FIRE) || (note->type & NOTE_FLAG_OPPONENT) != i)
					continue;
...
static void Stage_DrawNotes(void)
{
	...
	for (Note *note = stage.cur_note; note->pos != 0xFFFF; note++)
	{
		...
		if (y < FIXED_DEC(-16 - SCREEN_HEIGHT2, 1))
		{
			...
			if (!(note->type & (bot | NOTE_FLAG_HIT | NOTE_FLAG_FIRE | NOTE_FLAG_THUNDER)))
			{
...
static void Stage_LoadChart(void)
{
	...
	for (Note *note = stage.notes; note->pos != 0xFFFF; note++)
	{
		if (note->type & (NOTE_FLAG_SUSTAIN | NOTE_FLAG_THUNDER | NOTE_FLAG_FIRE))
			continue;
			...
			switch (stage.mode)
			{
				...
				case StageMode_Swap:
				{
					...
					for (Note *note = stage.cur_note;; note++)
					{
						...
						//Opponent note hits
						if (playing && (note->type & NOTE_FLAG_OPPONENT) && !(note->type & (NOTE_FLAG_HIT | NOTE_FLAG_FIRE | NOTE_FLAG_THUNDER)))
						{
							//Opponent hits note
							Stage_StartVocal();
	...
	else if (!(note->type & (NOTE_FLAG_FIRE | NOTE_FLAG_THUNDER)))
	{
		...
		if (hit_type == 0)
		{
			for (int i = 0; i < 3; i++)
			{
				//Create splash object
				Obj_Splash *splash = Obj_Splash_New(
					note_x[(note->type & 0x7)]  FIXED_DEC(bfnotex,1) +,
					note_y * (stage.downscroll ? -1 : 1),
					type
				);
				if (splash != NULL)
